...
export class PayloadRabbitMqTransport implements FetcherBrokerTransport {
  ...
  async assertTopology(routes: readonly WorkerRoute[]): Promise<void> {
    // RabbitMQ topology is provisioned and drift-checked centrally by the
    // backend baseline. Runtime stage identities intentionally have no
    // configure permission; consuming and publishing provide fail-closed
    // proof that the required queue, exchange, and binding exist.
    void routes;
    await this.ensureChannel();
  }
