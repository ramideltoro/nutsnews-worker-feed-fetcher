...
Production mode constructs a bounded PostgreSQL pool from the protected runtime URL and probes backend-owned `worker_uplift_fetcher.state_contract` version 1 plus every required table before broker consumption. It also requires production identities from the HTTP, DNS, RabbitMQ, and state-store adapters; a mixed or unknown set keeps consumption disabled and is exported truthfully. A missing URL at adapter construction selects an explicit `unsupported` store. A missing, stale, or unhealthy contract keeps consumption disabled; liveness, startup, and metrics remain available. Test mode alone may use the `local-memory` adapter.

The adapter implements leased inbox claims; transactional outcome/version/health persistence; and a durable canonicalization outbox. Outcome rows carry a 30-day `redact_after` boundary with an ordered cleanup index. The complete validated broker command is committed before publication. A failed publish releases the record to `retrying`; a new owner republishes the stored stable command, and confirmation is checked by candidate, idempotency key, owner, and message ID. The handler advances successful fetch metadata only after all candidate publications are confirmed, so a partial-fan-out retry skips completed candidates and resumes the missing remainder. The HTTP reconciler lists only bounded, aged pending commands and requires bearer authentication plus protected apply confirmation before replay.

The application does not run DDL. Its SQL migration is a test/reference representation of the contract. The backend repository owns the ordered additive production migration, schema/database role, grants, and protected connection-string injection.

## Safety Bounds

`NUTSNEWS_FETCHER_CONCURRENCY` caps concurrent feed-fetch handlers. `NUTSNEWS_FETCHER_PREFETCH` must be greater than or equal to concurrency and defaults to the same value so broker delivery does not queue a second request-timeout wave behind the active handlers during shutdown. Consumer cancellation is bounded, queued work is rejected on forced close, and the total stop operation remains bounded by `NUTSNEWS_FETCHER_SHUTDOWN_TIMEOUT_MS`. PostgreSQL pool size, query/connect timeout, and claim lease duration are bounded; the database timeout cannot exceed the startup-probe timeout in production. HTTP policy settings define connect/read/total timeouts, redirect count, accepted content types, streaming response-size limits, and the maximum safe `Retry-After` delay retained for transient failures.
...
The canonical fetch-stage metrics map the shared processor's single completion event to one of `success`, `duplicate`, `invalid`, `retry`, or `dlq`. Both success and duplicate are terminal successes and advance last-success. The latency histogram observes the processor's measured completion duration once in seconds using fixed cumulative buckets through the 30-second SLO boundary and up to 300 seconds; missing durations never manufacture observations.

Configured structured-log, telemetry, and metric sinks are independently wrapped as best-effort observers before broker, processor, ingestion, health, or shutdown use. Rejected observations cannot change acknowledgement, idempotency, retry, or DLQ state, and one rejected sink cannot starve the remaining fan-out. Dependency duration has one path: a real finite duration on `runtime.dependency.observed`; no service wrapper exposes the removed direct-latency method. Runtime 0.5 receives duration-free lifecycle clones for counters while this service owns the fixed-bucket seconds histograms, so no millisecond summaries escape. Health events continue to the log branch but are suppressed on the shared-metrics branch because the service owns a cold-start-seeded health family. Failed or disabled collection emits an explicit bounded status series.

## Idempotency

Runtime message idempotency acks repeated deliveries after a successful handler completion. The current PostgreSQL owner is the envelope message ID: it protects distinct-owner lease reclamation but cannot fence two executions of the same redelivered message. Production ownership therefore remains blocked until Runtime 1 supplies a unique claim token and compare-and-set completion, ambiguous claim/completion conformance passes, and the 30-minute shadow lease is reduced to no more than five minutes. Fetch metadata stores `ETag`, `Last-Modified`, and a content fingerprint per feed, while genuine upstream fetch outcomes remain in the outcome ledger; internal database/broker failures do not alter source-feed health. Candidate publication uses stable IDs derived from feed ID, source item ID, canonical URL, and content fingerprint; already confirmed candidate IDs are skipped, and retry/reconciliation republishes the original durable outbox command. Concurrent reconciliation apply also remains blocked until selection acquires an atomic replay claim.
